...
import { setBreadcrumbList } from "@/redux/modules/breadcrumb/actions";
...
const LayoutMenu = (props: any) => {
	...
	const formatMenuData = (menuList: Menu.MenuOptions[], newArr: MenuItem[] = []) => {
		menuList.forEach((item: Menu.MenuOptions) => {
			if (!item?.children?.length) return newArr.push(getItem(item.title, item.path, addIcon(item.icon!)));
			newArr.push(getItem(item.title, item.path, addIcon(item.icon!), formatMenuData(item.children)));
		});
		return newArr;
	};
	9;
	...
	const getMenuData = async () => {
		...
		try {
			...
			// if(!data) return
			setMenuList(formatMenuData(data));
